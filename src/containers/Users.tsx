...
import { startPage, defaultLimit } from '../constants/constants';
...
const Users: FC = () => {
    ...
    const { data, error: getUsersErrror, isLoading: getUsersIsLoading } = usersApiSlice.useGetUsersQuery({page, limit});
    const [ deleteUser, { data: deleted, error, isLoading }] = usersApiSlice.useDeleteUserMutation();
    ...
    return (
      <Box sx={{width: '100%'}}>
        <Box sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          {data?.totalPages && <TableContainer component={Paper}>
              ...
              <TableBody>
                {data?.users?.map((user:any) => (
                  ...
                  >
                    ...
                    <TableCell align="center">{currentUser?.id && user?.id &&<DotMenu
                      id={user.id}
